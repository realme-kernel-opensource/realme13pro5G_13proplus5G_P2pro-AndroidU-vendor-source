	...
	if ((prop_node->feature >= SDE_CP_CRTC_MAX_FEATURES) ||
		...
		for (i = 0; i < num_mixers && !ret; i++) {
			...
			if (oplus_ofp_is_supported()) {
				...
				} else if (prop_node->feature == SDE_CP_CRTC_DSPP_IGC) {
					oplus_ofp_bypass_dspp_igc(&hw_cfg, sde_crtc);
				} else if (prop_node->feature == SDE_CP_CRTC_DSPP_GC) {
					oplus_ofp_bypass_dspp_gc(&hw_cfg, sde_crtc);
				}
			}
#endif /* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
...
void sde_cp_crtc_gc_change(struct drm_crtc *crtc_drm)
{
        struct sde_cp_node *prop_node = NULL;
        struct sde_crtc *crtc;
        ...
        mutex_lock(&crtc->crtc_cp_lock);
        list_for_each_entry(prop_node, &crtc->cp_feature_list, cp_feature_list) {
                if (prop_node->feature != SDE_CP_CRTC_DSPP_GC)
                        continue;
                ...
                pr_err("oplus_gc: %s %d prop_node->feature=%d\n", __func__, __LINE__, SDE_CP_CRTC_DSPP_GC);
                _sde_cp_update_list(prop_node, crtc, true);
        }
...
void sde_cp_crtc_igc_change(struct drm_crtc *crtc_drm)
{
        struct sde_cp_node *prop_node = NULL;
        struct sde_crtc *crtc;
        ...
        mutex_lock(&crtc->crtc_cp_lock);
        list_for_each_entry(prop_node, &crtc->cp_feature_list, cp_feature_list) {
                if (prop_node->feature != SDE_CP_CRTC_DSPP_IGC)
                        continue;
                ...
                pr_err("oplus_igc: %s %d prop_node->feature=%d\n", __func__, __LINE__, SDE_CP_CRTC_DSPP_IGC);
                _sde_cp_update_list(prop_node, crtc, true);
        }

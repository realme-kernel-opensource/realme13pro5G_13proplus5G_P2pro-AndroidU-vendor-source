...
int oplus_display_panel_set_dynamic_osc_clock(void *data)
{
	...
	if (!rc) {
		...
		if ((!strcmp(display->panel->name, "coffee tianma ili7838a dsc cmd mode panel"))
			|| (!strcmp(display->panel->name, "amber wxn ili7838a dsc cmd mode panel")))
...
int oplus_display_panel_set_demura_amber()
{
	u32 bl_lvl = 0;
	int rc = 0;
	struct dsi_display *display = NULL;
	...
	/* if not thisAB790 panel, return */
	if (strcmp(panel->oplus_priv.vendor_name, "AB790")) {
		return rc;
	}

	/*just need setting tm*/
	if (str_equal(panel->name, "amber wxn ili7838a dsc cmd mode panel")) {
		return rc;
	}
	...
	mutex_lock(&panel->panel_lock);
	if (panel->power_mode != SDE_MODE_DPMS_OFF) {
		if (bl_lvl >= 0xDBB && panel->demura_flag != 1) {
			SDE_ATRACE_BEGIN("oplus_display_panel_set_demura_amber");
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_SW_SEOF);
			rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_DEMURR_BDVDBB);
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_NONE);
			panel->demura_flag = 1;
			OFP_INFO("demuradbb_lvl%x,demura_flag%d", bl_lvl , panel->demura_flag);
			SDE_ATRACE_END("oplus_display_panel_set_demura_amber");
		} else if (bl_lvl < 0xDBB && bl_lvl >= 0x562 && panel->demura_flag != 2) {
			SDE_ATRACE_BEGIN("oplus_display_panel_set_demura_amber");
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_SW_SEOF);
			rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_DEMURR_BDV562);
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_NONE);
			panel->demura_flag = 2;
			OFP_INFO("demura562_lvl%x,demura_flag%d", bl_lvl , panel->demura_flag);
			SDE_ATRACE_END("oplus_display_panel_set_demura_amber");
		} else if (bl_lvl < 0x562 && bl_lvl >= 0x3F2 && panel->demura_flag != 3) {
			SDE_ATRACE_BEGIN("oplus_display_panel_set_demua");
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_SW_SEOF);
			rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_DEMURR_BDV3F2);
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_NONE);
			panel->demura_flag = 3;
			OFP_INFO("demura3f2_lvl%x,demura_flag%d", bl_lvl , panel->demura_flag);
			SDE_ATRACE_END("oplus_display_panel_set_demura_amber");
		} else if (bl_lvl < 0x3F2 && bl_lvl >= 0x1F6 && panel->demura_flag != 4) {
			SDE_ATRACE_BEGIN("oplus_display_panel_set_demura_amber");
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_SW_SEOF);
			rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_DEMURR_BDV1F6);
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_NONE);
			panel->demura_flag = 4;
			OFP_INFO("demura1f6_lvl%x,demura_flag%d", bl_lvl , panel->demura_flag);
			SDE_ATRACE_END("oplus_display_panel_set_demura_amber");
		} else if (bl_lvl < 0x1F6 && bl_lvl >= 0x008 && panel->demura_flag != 5) {
			SDE_ATRACE_BEGIN("oplus_display_panel_set_demura_amber");
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_SW_SEOF);
			rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_DEMURR_BDV008);
			rc |= dsi_display_override_dma_cmd_trig(display, DSI_TRIGGER_NONE);
			panel->demura_flag = 5;
			OFP_INFO("demura008_lvl%x,demura_flag%d", bl_lvl , panel->demura_flag);
			SDE_ATRACE_END("oplus_display_panel_set_demura_amber");
		} else {
		}
	}
	...
	if (rc) {
		DSI_ERR("failed to oplus_display_panel_set_demura_amber, rc = %d\n", rc);
	}

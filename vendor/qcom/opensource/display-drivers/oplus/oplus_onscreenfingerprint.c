...
int oplus_ofp_init(void *dsi_panel)
{
	...
	if (oplus_ofp_is_supported()) {
		...
		p_oplus_ofp_params->need_to_bypass_gc = utils->read_bool(utils->data, "oplus,ofp-need-to-bypass-gc");
                OFP_INFO("need_to_bypass_gc:%d\n", p_oplus_ofp_params->need_to_bypass_gc);

		p_oplus_ofp_params->need_to_bypass_igc = utils->read_bool(utils->data, "oplus,ofp-need-to-bypass-igc");
                OFP_INFO("need_to_bypass_igc:%d\n", p_oplus_ofp_params->need_to_bypass_igc);
...
int oplus_ofp_property_update(void *sde_connector, void *sde_connector_state, int prop_id, uint64_t prop_val)
{
	...
	case CONNECTOR_PROP_HBM_ENABLE:
		if (prop_val != p_oplus_ofp_params->hbm_enable) {
			...
			if (!strcmp(display->panel->name, "amber wxn ili7838a dsc cmd mode panel") ||
					!strcmp(display->panel->name, "amber tianma nt37705 dsc cmd mode panel"))
				oplus_underbrightness_alpha = prop_val & OPLUS_OFP_PROPERTY_DIM_LAYER;
		}
...
bool oplus_ofp_need_gc_change(void *s_crtc)
{
        bool need_gc_change = false;
        bool new_bypass_pq_status = false;
        static bool last_bypass_pq_status = false;
        struct sde_crtc *sde_crtc = s_crtc;
        struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params(oplus_ofp_display_id);
	...
	if (!p_oplus_ofp_params->need_to_bypass_gc) {
		return false;
	}

        OPLUS_OFP_TRACE_BEGIN("oplus_ofp_need_gc_change");
        ...
        */
        if (new_bypass_pq_status != last_bypass_pq_status) {
                need_gc_change = true;
                OFP_INFO("need gc change\n");
        }

        last_bypass_pq_status = new_bypass_pq_status;

        OPLUS_OFP_TRACE_END("oplus_ofp_need_gc_change");

        OFP_DEBUG("end\n");

        return need_gc_change;
}

bool oplus_ofp_need_igc_change(void *s_crtc)
{
        bool need_igc_change = false;
        bool new_bypass_pq_status = false;
        static bool last_bypass_pq_status = false;
        struct sde_crtc *sde_crtc = s_crtc;
        struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params(oplus_ofp_display_id);
        ...
        if (!p_oplus_ofp_params->need_to_bypass_igc) {
			return false;
        }

        OPLUS_OFP_TRACE_BEGIN("oplus_ofp_need_igc_change");
        ...
        */
        if (new_bypass_pq_status != last_bypass_pq_status) {
                need_igc_change = true;
                OFP_INFO("need gc change\n");
        }

        last_bypass_pq_status = new_bypass_pq_status;

        OPLUS_OFP_TRACE_END("oplus_ofp_need_igc_change");

        OFP_DEBUG("end\n");

        return need_igc_change;
}
...
int oplus_ofp_bypass_dspp_gc(void *sde_hw_cp_cfg, void *s_crtc)
{
        struct sde_hw_cp_cfg *hw_cfg = sde_hw_cp_cfg;
        struct sde_crtc *sde_crtc = s_crtc;
        struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params(oplus_ofp_display_id);
        ...
        OPLUS_OFP_TRACE_BEGIN("oplus_ofp_bypass_dspp_gc");


        if (oplus_ofp_need_to_bypass_pq(sde_crtc) && p_oplus_ofp_params->need_to_bypass_gc) {
                hw_cfg->payload = NULL;
                OFP_DEBUG("bypass dspp gamut\n");
        }

        OPLUS_OFP_TRACE_END("oplus_ofp_bypass_dspp_gc");
...
int oplus_ofp_bypass_dspp_igc(void *sde_hw_cp_cfg, void *s_crtc)
{
        struct sde_hw_cp_cfg *hw_cfg = sde_hw_cp_cfg;
        struct sde_crtc *sde_crtc = s_crtc;
        struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params(oplus_ofp_display_id);
        ...
        OPLUS_OFP_TRACE_BEGIN("oplus_ofp_bypass_dspp_igc");


        if (oplus_ofp_need_to_bypass_pq(sde_crtc) && p_oplus_ofp_params->need_to_bypass_igc) {
                hw_cfg->payload = NULL;
                OFP_DEBUG("bypass dspp igc\n");
        }

        OPLUS_OFP_TRACE_END("oplus_ofp_bypass_dspp_igc");

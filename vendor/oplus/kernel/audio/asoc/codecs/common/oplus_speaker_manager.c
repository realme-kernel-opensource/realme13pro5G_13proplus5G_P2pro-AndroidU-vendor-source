...
int oplus_spkr_pa_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	...
	if (speaker_device != NULL) {
		l_spk_status = speaker_device->speaker_status;
		pr_err("%s, %d, l_spk_status = %d\n", __func__, __LINE__, l_spk_status);
	}

	speaker_device = get_speaker_dev(R_SPK);
	if (speaker_device != NULL) {
		r_spk_status = speaker_device->speaker_status;
		pr_err("%s, %d, r_spk_status = %d\n", __func__, __LINE__, r_spk_status);
	}

	switch (event) {
	case SND_SOC_DAPM_POST_PMU :
			if (contrl_status->chipset == ALL_SPK) {
				if (l_spk_status == 0 && r_spk_status == 0) {

...
static void oplus_gauge_check_ratio_status(struct oplus_mms_gauge *chip)
{
	if (!chip || !chip->deep_spec.support || chip->ui_soc < chip->deep_spec.config.soc)
		return;
...
static void oplus_gauge_deep_dischg_check(struct oplus_mms_gauge *chip)
{
	...
	if (!charging)
		oplus_gauge_check_ratio_status(chip);
...
static void oplus_gauge_deep_track_work(struct work_struct *work)
{
	...
	index = snprintf(reg_info, REG_INFO_LEN, "$$dischg_counts@@%d$$count_thr@@%d$$count_cali@@%d$$cc@@%d$$ratio@@%d"
		"$$vbat_uv@@%d$$vbat_min@@%d$$tbat@@%d$$ui_soc@@%d$$ibat_ma@@%d$$bybid@@%d$$batt_id@@%d$$sili_err@@%d",
		chip->deep_spec.counts, chip->deep_spec.config.count_thr, chip->deep_spec.config.count_cali, chip->deep_spec.cc,
		chip->deep_spec.ratio, chip->deep_spec.config.uv_thr, vbat_min_mv, batt_temp,

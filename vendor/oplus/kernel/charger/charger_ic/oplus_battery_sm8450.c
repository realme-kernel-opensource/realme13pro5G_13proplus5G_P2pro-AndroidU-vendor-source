...
void oplus_get_props_from_adsp_by_buffer(void)
{
	...
	if (oplus_vooc_get_fastchg_started()) {
		if (chip->ffc_temp_status == FFC_TEMP_STATUS__LOW) {
			...
			full_vol = chip->limits.ffc2_normal_vfloat_sw_limit + bcdev->ffc_full_delta_vfloat_mv;
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,ffc_full_delta_vfloat_mv",
			&bcdev->ffc_full_delta_vfloat_mv);
	if (rc < 0) {
		bcdev->ffc_full_delta_vfloat_mv = FFC_FULL_DELTA_VFLOAT_MV;
	}

	bcdev->pmic_is_pm7250b = of_property_read_bool(bcdev->dev->of_node, "qcom,pmic-is-pm7250b");
	chg_err("pmic_is_pm7250b:%d,ffc_full_delta_iterm_ma:%d,ffc_full_delta_vfloat_mv:%d\n", bcdev->pmic_is_pm7250b,
		bcdev->ffc_full_delta_iterm_ma, bcdev->ffc_full_delta_vfloat_mv);
